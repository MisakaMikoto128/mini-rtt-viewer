//! `--demo-log` 演示/测试数据源:无设备时验证滚动、断行、UTF-8 渲染。
//!
//! - 跳过单实例互斥由 main 负责:demo 模式允许与真实实例并存(它不加载
//!   JLinkARM.dll,不会抢 J-Link)
//! - 发送与上屏各打一行微秒时间戳(stderr),用于测量显示节奏是否与
//!   发送节奏一致;非 demo 模式 `T0` 为空,零开销
...
/// 上屏打点(timer flush 处调用;非 demo 模式为空)
pub fn trace_flush(lines: usize) {
    if let Some(t0) = T0.get() {
        eprintln!("[flush] lines={lines} t={}us", t0.elapsed().as_micros());
    }
}
